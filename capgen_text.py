...
import flickr8k
import flickr30k
import coco_text as coco

from IPython import embed

logging.basicConfig(level=logging.DEBUG, filename='attention.log', mode='w')
...
def param_init_lstm_cond(options, params, prefix='lstm_cond', nin=None, dim=None, dimctx=None, dimtex=None):
    if nin is None:
        nin = options['dim']
    if dim is None:
        dim = options['dim']
    ...
    Wt_att = norm_weight(dimtex, ortho=False)
    params[_p(prefix,'Wt_att')] = Wt_att

    # attention: LSTM -> hidden
    Wd_att = norm_weight(dim,dimctx)
    params[_p(prefix,'Wd_att')] = Wd_att

    Wdt_att = norm_weight(dim, dimtex)
    params[_p(prefix,'Wdt_att')] = Wdt_att

    # attention: hidden bias
    b_att = numpy.zeros((dimctx,)).astype('float32')
    params[_p(prefix,'b_att')] = b_att

    bt_att = numpy.zeros((dimtex,)).astype('float32')
    ...
    if options['n_layers_att'] > 1:
        for lidx in xrange(1, options['n_layers_att']):
            ...
            params[_p(prefix,'Wt_att_%d'%lidx)] = ortho_weight(dimtex)
            params[_p(prefix,'bt_att_%d'%lidx)] = numpy.zeros((dimtex,)).astype('float32')            
    ...
    Ut_att = norm_weight(dimtex,1)
    params[_p(prefix,'Ut_att')] = Ut_att
    ct_att = numpy.zeros((1,)).astype('float32')
    params[_p(prefix, 'ct_att')] = ct_att
    ...
    if options['selector']:
        ...
        Wt_sel = norm_weight(dim, 1)
        params[_p(prefix, 'Wt_sel')] = Wt_sel
        bt_sel = numpy.float32(0.)
        params[_p(prefix, 'bt_sel')] = bt_sel

    return params

def lstm_cond_layer(tparams, state_below, options, prefix='lstm',
                    mask=None, context=None, text=None, one_step=False,
    ...
    ptex_ = tensor.dot(text, tparams[_p(prefix,'Wt_att')]) + tparams[_p(prefix,'bt_att')]
    if options['n_layers_att'] > 1:
        for lidx in xrange(1, options['n_layers_att']):
            pctx_ = tensor.dot(pctx_, tparams[_p(prefix,'W_att_%d'%lidx)])+tparams[_p(prefix, 'b_att_%d'%lidx)]
            ptex_ = tensor.dot(ptex_, tparams[_p(prefix,'Wt_att_%d'%lidx)])+tparams[_p(prefix, 'bt_att_%d'%lidx)]
    ...
    def _step(m_, x_, h_, c_, a_, as_, ct_, t_, ts_, tt_, pctx_, ptex_, dp_=None, dp_att_=None):
        ...
        pstate_ = pstate_ + tensor.dot(h_, tparams[_p(prefix,'Wdt_att')])
        ptex_ = ptex_ + pstate_[:,None,:]
        ptex_list = []
        ptex_list.append(ptex_)
        ptex_ = tanh(ptex_)
        tau = tensor.dot(ptex_, tparams[_p(prefix,'Ut_att')])+tparams[_p(prefix, 'ct_att')]
        tau_pre = tau
        tau_shp = tau.shape
        ...
        if options['attn_type'] == 'deterministic':
            ...
            tau = tensor.nnet.softmax(tau.reshape([tau_shp[0],tau_shp[1]])) # softmax
            tex_ = (text * tau[:,:,None]).sum(1) # current context
            tau_sample = tau # you can return something else reasonable here to debug
        else:
        ...
        if options['selector']:
            ...
            selt_ = tensor.nnet.sigmoid(tensor.dot(h_, tparams[_p(prefix, 'Wt_sel')])+tparams[_p(prefix,'bt_sel')])
            selt_ = selt_.reshape([selt_.shape[0]])
            tex_ = selt_[:,None] * tex_

        preact = tensor.dot(h_, tparams[_p(prefix, 'U')])
        preact += x_
        preact += tensor.dot(ctx_, tparams[_p(prefix, 'Wc')])
        preact += tensor.dot(tex_, tparams[_p(prefix, 'Wt')])
        ...
        rval = [h, c, alpha, alpha_sample, ctx_, tau, tau_sample, tex_]
        if options['selector']:
            rval += [sel_, selt_]
        rval += [pstate_, pctx_, ptex_, i, f, o, preact, alpha_pre, tau_pre]+pctx_list+ptex_list
        return rval

    if options['use_dropout_lstm']:
        if options['selector']:
            _step0 = lambda m_, x_, dp_, h_, c_, a_, as_, ct_, t_, ts_, tt_, sel_, selt_, pctx_, ptex_: \
                            _step(m_, x_, h_, c_, a_, as_, ct_, t_, ts_, tt_, pctx_, ptex_, dp_)
        else:
            _step0 = lambda m_, x_, dp_, h_, c_, a_, as_, ct_, t_, ts_, tt_, pctx_, ptex_: \
                            _step(m_, x_, h_, c_, a_, as_, ct_, t_, ts_, tt_, pctx_, ptex_, dp_)
        ...
        if options['selector']:
            _step0 = lambda m_, x_, h_, c_, a_, as_, ct_, t_, ts_, tt_, sel_, selt_, pctx_, ptex_: _step(m_, x_, h_, c_, a_, as_, ct_, t_, ts_, tt_, pctx_, ptex_)
        else:
            _step0 = lambda m_, x_, h_, c_, a_, as_, ct_, t_, ts_, tt_, pctx_, ptex_: _step(m_, x_, h_, c_, a_, as_, ct_, t_, ts_, tt_, pctx_, ptex_)

    if one_step:
        if options['use_dropout_lstm']:
            if options['selector']:
                rval = _step0(mask, state_below, dp_mask, init_state, init_memory, None, None, None, None, None, None, None, None, pctx_, ptex_)
            else:
                rval = _step0(mask, state_below, dp_mask, init_state, init_memory, None, None, None, None, None, None, pctx_, ptex_)
        else:
            if options['selector']:
                rval = _step0(mask, state_below, init_state, init_memory, None, None, None, None, None, None, None, None, pctx_, ptex_)
            else:
                rval = _step0(mask, state_below, init_state, init_memory, None, None, None, None, None, None, pctx_, ptex_)
        return rval
    else:
        ...
        outputs_info = [init_state,                                     # h
                        ...
                        tensor.alloc(0., n_samples, ptex_.shape[1]),    # t
                        tensor.alloc(0., n_samples, ptex_.shape[1]),    # ts
                        tensor.alloc(0., n_samples, text.shape[2])]     # tt
        if options['selector']:
            outputs_info += [tensor.alloc(0., n_samples)]               # sel
            outputs_info += [tensor.alloc(0., n_samples)]               # selt
        outputs_info += [None,                                          # pstate
                         ...
                         None,                                          # preact
                         None,                                          # alpha_pre
                         None]+[None, None] # *options['n_layers_att']  # tau_pre + [pctx_list, ptex_list]
        rval, updates = theano.scan(_step0,
                                    sequences=seqs,
                                    outputs_info=outputs_info,
                                    non_sequences=[pctx_, ptex_],       # pctx, ptex
                                    name=_p(prefix, '_layers'),
                                    n_steps=nsteps, profile=False)
        return rval, updates
...
def init_params(options):
    ...
    if options['tex2out']:
        params = get_layer('ff')[0](options, params, prefix='ff_logit_tex', nin=tex_dim, nout=options['dim_word'])
...
def build_model(tparams, options, sampling=True):
    ...
    tex = tensor.tensor3('tex', dtype='float32')
    ...
    if options['lstm_encoder_context']:
        tex_fwd = get_layer('lstm')[1](tparams, tex.dimshuffle(1,0,2),
    ...
    mean = ctx0.mean(1) + tex0.mean(1)
    for lidx in xrange(1, options['n_layers_init']):
        mean = get_layer('ff')[1](tparams, mean, options,
                                      prefix='ff_init_%d'%lidx, activ='rectifier')
        if options['use_dropout']:
            mean = dropout_layer(mean, use_noise, trng)

    init_state = get_layer('ff')[1](tparams, mean, options, prefix='ff_state', activ='tanh')
    init_memory = get_layer('ff')[1](tparams, mean, options, prefix='ff_memory', activ='tanh')
    # lstm decoder
    # [equation (1), (2), (3) in section 3.1.2]
    attn_updates = []
    # proj = [h, c, alpha, alpha_sample, ctx_, tau, tau_sample, tex_]
    ...
    if options['n_layers_lstm'] > 1:
        for lidx in xrange(1, options['n_layers_lstm']):
            init_state = get_layer('ff')[1](tparams, mean, options, prefix='ff_state_%d'%lidx, activ='tanh')
            init_memory = get_layer('ff')[1](tparams, mean, options, prefix='ff_memory_%d'%lidx, activ='tanh')
    ...
    if options['tex2out']:
        logit += get_layer('ff')[1](tparams, texs, options, prefix='ff_logit_tex', activ='linear')
    logit = tanh(logit)
    if options['use_dropout']:
    ...
    return trng, use_noise, [x, mask, ctx, tex], alphas, alphas_sample, taus, taus_sample, cost, opt_outs
...
def build_sampler(tparams, options, use_noise, trng, sampling=True):
    ...
    if options['n_layers_lstm'] > 1:
        for lidx in xrange(1, options['n_layers_lstm']):
            init_state.append(get_layer('ff')[1](tparams, mean, options, prefix='ff_state_%d'%lidx, activ='tanh'))
    ...
    next_state, next_memory, ctxs, texs = [proj[0]], [proj[1]], [proj[4]], [proj[7]]
    proj_h = proj[0]
    if options['n_layers_lstm'] > 1:
        for lidx in xrange(1, options['n_layers_lstm']):
    ...
    if options['tex2out']:
        logit += get_layer('ff')[1](tparams, texs[-1], options, prefix='ff_logit_tex', activ='linear')
    logit = tanh(logit)
    if options['use_dropout']:
        logit = dropout_layer(logit, use_noise, trng)
    if options['n_layers_out'] > 1:
...
# generate sample
def gen_sample(tparams, f_init, f_next, ctx0, tex0, options, trng=None, k=1, maxlen=30, stochastic=False):
    ...
    for ii in xrange(maxlen):
        ...
        rval = f_next(*([next_w, ctx0, tex0]+next_state+next_memory))
        next_p = rval[0]
        next_w = rval[1]
...
def pred_probs(f_log_probs, options, worddict, prepare_data, data, iterator, verbose=False):
    ...
    for _, valid_index in iterator:
        x, mask, ctx, tex = prepare_data([data[0][t] for t in valid_index],
                                     data[1],
                                     data[2],
                                     worddict,
                                     maxlen=None,
    ...
    try:
        ...
        for eidx in xrange(max_epochs):
            ...
            for caps in train_iter:
                ...
                ud_start = time.time()
                cost = f_grad_shared(x, mask, ctx, tex)
                f_update(lrate)
                ...
                if numpy.mod(uidx, sampleFreq) == 0:
                    ...
                    for jj in xrange(numpy.minimum(10, len(caps))):
                        sample, score = gen_sample(tparams, f_init, f_next, ctx_s[jj], tex_s[jj], model_options,
